...
fn main() {
    ...
    let headings = Regex::new(r"([0-9]*\.)*[0-9] (?<fromto>(?<from>.*) to (?<to>[^\r\n]*))\r").unwrap();
    ...
    let change_sets: Vec<_> = change_sets_regex
        ...
        .map(|c| {
            ...
            let changes: Vec<_> = re
                ...
                .map(|c| {
                    let before: Vec<String> = c.name("before").unwrap().as_str().to_string().split_terminator(" ").map(|c| {c.to_string()}).collect();
                    let mut after: Vec<String> = c.name("after").unwrap().as_str().to_string().split_terminator(" ").map(|c| {c.to_string()}).collect();
                    let env = c.name("env").unwrap().as_str().to_string();
...
fn finish(f: String)  -> std::io::Result<()> {
    std::fs::write::<&str, &[u8]>(r"C:/Users\ensel\Documents\nils\Programming\indexdiachronica\INDEX\ouput.txt", f.as_ref())?;
